...
const UserCard = ({ user }) => {
    ...
    // function to handle connections request
    ...
    return (
        <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md hover:shadow-xl transition '>
            <div>
                <img src={user.profile_picture} alt="" className='rounded-full w-16 shadow-md mx-auto' />
                <p className='mt-4 text-center font-semibold'>{user.full_name}</p>
            ...
            <div className='flex items-center justify-center gap-2 mt-4 text-xs text-gray-600'>
                {/* displaying location of user */}
                <div className='flex items-center gap-1 border border-gray-300 rounded-full px-3 py-1'>
                    <MapPin className='w-4 h-4' /> {user.location}
                </div>
                {/* displaying number of followers of user */}
                <div className='flex items-center gap-1 border border-gray-300 rounded-full px-3 py-1'>
